...
MetasCarga.prototype.init = function() {
    this.handle();
};

MetasCarga.prototype.handle = function() {
    var obj = this;

    obj.inputFile.fileinput({
        theme: 'fa',
        language: 'es',
        uploadUrl: obj.uri + "metas/subir-excel",
        ...
        if (data.response.status) {
            ...
            Promotick.toast.error(data.response.message, 'Carga de metas');
        }
...
MetasCarga.prototype.mostrarResumen = function(response) {
    var obj = this;
    if (!obj.resumen) {
        obj.resumen = true;

        Promotick.render.post({
            url: obj.uri + 'metas/viewResumenCargaMetas',
