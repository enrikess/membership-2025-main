function ProductosDetalle(uri, registro) {
    ...
    this.imagenPrincipalApp = $('#imagenPrincipalApp');
    this.imagenDetalleApp = $('#imagenDetalleApp');
    ...
    this.pdfAden = $('#pdfAden');
    this.tabPdf = $('#tabPdf');
    this.btnGuardarPDF = $('#btnGuardarPDF');

    this.puntosRegular = $('#puntosRegular');
    this.slcTagProducto = $('#tagProducto');
...
ProductosDetalle.prototype.handle = function() {
    ...
    var pdfDefecto = obj.pdfAden.attr('data-pdf');

    obj.makeFileInput(obj.imagenPrincipal, '#principalError');
    obj.makeFileInput(obj.imagenDetalle, '#detalleError');
    obj.makeFileInput(obj.imagenPrincipalApp, '#principalError');
    obj.makeFileInput(obj.imagenDetalleApp, '#detalleError');
    obj.makeFileInput(obj.pdfAden, '#pdfError');
    ...
    obj.btnGuardarPDF.on('click', function(e) {
        e.preventDefault();

        var objValidatePdf = obj.validateImagenes(this);
        if (objValidatePdf.status) {
            obj.cargaPdf(objValidatePdf);
        } else if (objValidatePdf.mensaje != null) {
            Promotick.toast.warning(objValidatePdf.mensaje, 'Carga de pdf')
        }
    });
...
ProductosDetalle.prototype.mostrarCargaPdf = function () {
...
ProductosDetalle.prototype.actualizarEstado = function(idProductoCatalogo, checked, $checkbox) {
    ...
    var request = {
        ...
        estadoProductoCatalogo : checked ? 1 : 0
    };
...
ProductosDetalle.prototype.makeFileInput = function(instance, idError) {
    ...
    var archivoDefault = instance.attr('data-pdf-default');
    var archivo = instance.attr('data-pdf');

    var allowedFileExtensions = ["jpg", "png", "gif", "jpeg", "pdf"];
    ...
    instance.fileinput({
        ...
        defaultPreviewContent: (function() {
            if (archivo && typeof archivo === 'string' && allowedFileExtensions.includes(archivo.split('.').pop().toLowerCase())) {
                return '<embed src="' + archivo + '" type="application/pdf" style="width:100%; height:250px;">';
            } else {
                return '<img src="' + imagen + '" alt="" style="max-height: 250px">';
            }
        })(),
    ...
    var $objPreview = instance.parent().parent().find('.file-preview');
    var $objFile = $objPreview.find('.file-default-preview');
    var $objImg = $objFile.find('img');

    if (archivo && typeof archivo === 'string' && allowedFileExtensions.includes(archivo.split('.').pop().toLowerCase())) {
        var pdf = new Image();
        pdf.src = archivo;
        pdf.onload = function() {
            $objImg.attr('src', archivo);
        };
        pdf.onerror = function() {
            $objImg.attr('src', archivoDefault);
        };
    } else {
        var image = new Image();
        image.src = imagen;
        ...
        obj.gridCatalogos.dataTable({
            ...
            "initComplete":function(settings,json){
                if(json.data.length > 0){
                    $(document).find('#btnAgregarCatalogo')
                        .css('display','none');
                }
            },
            'columnDefs' : [
...
ProductosDetalle.prototype.validateInfo = function(e) {
    ...
    var tagsProducto = null;
    $(document).find('input.tagsProducto').each(function(i, e) {
        var value = $(e).val();
        if (value !== '') {
            if (tagsProducto == null) {
                tagsProducto = value;
            } else {
                tagsProducto += ',' + value;
            }
    ...
    //if (tagsProducto == null && obj.registro == 1) { tagsProducto = ""; }
    if (obj.formProducto.valid()) {
        if (terminos == null) {
            ...
            var imagenEnvioExpress = -1;

            if (obj.chkExpress.is(":checked")){
                imagenEnvioExpress = 1;
            }
            ...
            objValid.request = {
                ...
                puntosRegular : obj.puntosRegular.val().trim() == "" ? null : obj.puntosRegular.val(),
                precioRegular : obj.puntosProducto.val(),
                precioProducto : obj.precioProducto.val(),
                nombreTag : obj.nombreTag.val(),
                tipoProducto : {
                    idTipoProducto : obj.tipoProducto.val()
                },
                tagProducto : {
                    idTagProducto : obj.slcTagProducto.val()
                },
                marca : {
                ...
                tags : tagsProducto,
                imagenEnvioExpress : imagenEnvioExpress
            };
...
ProductosDetalle.prototype.validateImagenes = function(e) {
    ...
    // } else {
        ...
        $.each($('#imagenPrincipalApp')[0].files, function(i, file) {
            data.append('imagenPrincipalApp', file);
        });

        $.each($('#imagenDetalleApp')[0].files, function(i, file) {
            data.append('imagenDetalleApp', file);
        });

        $.each($('#pdfAden')[0].files, function(i, file) {
            data.append('pdfAden', file);
        });
...
ProductosDetalle.prototype.initValidations = function() {
    ...
    obj.formProducto.validate({
        ...
        messages : {
            ...
            tagProducto : {
                required : 'Seleccione una etiqueta para el producto',
                min : 'Seleccione una etiqueta para el producto',
            },
            categoriaProducto : {
...
ProductosDetalle.prototype.cargaPdf = function(objValidatePdf) {
    var obj = this;

    var loader = null;

    Promotick.ajax.post({
        url : obj.uri + 'catalogos/productos/cargar-pdf',
        data : objValidatePdf.formData,
        processData : false,
        contentType : false,
        messageError : true,
        messageTitle : 'Carga de pdf',
        before : function() {
            loader = Ladda.create(objValidatePdf.handle);
            loader.start();
        },
        success : function(response) {
            if (response.status) {
                Promotick.toast.success(response.message, 'Carga de pdf');
                $('html, body').animate({scrollTop:0}, 'slow');
            }
        },
        complete : function() {
...
ProductosDetalle.prototype.registrarProducto = function(validateInfo, validateImagenes, validateCatalogo) {
    ...
    validateImagenes.formData.append('idTagProducto', validateInfo.request.tagProducto.idTagProducto);
    ...
    if(validateInfo.request.tags == null) {
        validateImagenes.formData.delete('tags');
    }
    if(validateInfo.request.puntosRegular == null) {
        validateImagenes.formData.delete('puntosRegular');
    }
