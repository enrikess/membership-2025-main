...
@Slf4j
@Controller
@RequestMapping("administracion/banner-flotante")
public class BannerFlotanteController extends BaseController {

	private ApiS3Service apiS3Service;
	private CatalogoAdminService catalogoAdminService;

	@Autowired
	private BannerFlotanteController(ApiS3Service apiS3Service, CatalogoAdminService catalogoAdminService){
	...
	@GetMapping
	public String init() {
		return ConstantesAdminView.VIEW_ADMINISTRACION_BANNER_FLOTANTE;
	}
	...
	public PromotickResult actualizarEstado(@RequestBody Catalogo catalogo, PromotickResult promotickResult) {
		try {
			Integer tipoActualizar = UtilEnum.TIPO_ACTUALIZAR.BANNER_FLOTANTE.getCodigo();
			if (catalogo.getEstadoBannerFlotante().equals(UtilEnum.ESTADO.ACTIVO.getCodigo())) {
				catalogo.setEstadoBannerFlotante(UtilEnum.ESTADO.INACTIVO.getCodigo());
			} else if (catalogo.getEstadoBannerFlotante().equals(UtilEnum.ESTADO.INACTIVO.getCodigo())) {
				catalogo.setEstadoBannerFlotante(UtilEnum.ESTADO.ACTIVO.getCodigo());
			}
			Integer resultado = catalogoAdminService.actualizarEstadoCatalogo(catalogo.getIdCatalogo(), catalogo.getEstadoBannerFlotante(), tipoActualizar);
			evaluarResultado(resultado, promotickResult, "Banner flotante actualizado correctamente");
		} catch (Exception e) {
	...
	public String registrar(@PathVariable("idSegmento")Integer idSegmento, Model model){
		...
		if (catalogos.isEmpty()) {
			return "redirect:/administracion/banner-flotante";
		}
		model.addAttribute("objSegmento", catalogos.get(0));

		return ConstantesAdminView.VIEW_ADMINISTRACION_BANNER_FLOTANTE_EDITAR;
	}
	...
	public PromotickResult cargarImagenes(
			...
			@RequestParam(value = "urlBanner") String urlBanner,
            @RequestParam(value = "bannerImagen", required = false) MultipartFile bannerImagen,
		...
		try {
			...
			catalogo.setUrlBannerFlotante(urlBanner);
			if (bannerImagen != null) {
				Integer idImagenPrincipal = this.subirImagenS3(bannerImagen, usuario.getIdUsuario());
				if (idImagenPrincipal == null) {
					throw new Exception("Error al subir imagen popup");
				}
				catalogo.setBannerFlotante(bannerImagen.getOriginalFilename());
			}

			Integer resultado= catalogoAdminService.actualizarBannerFlotanteCatalogo(catalogo);
