...
public class CargaMetaController extends BaseController {
    ...
    @Autowired
    public CargaMetaController(ReporteAdminService reporteAdminService, CargaExcelAdminService cargaExcelAdminService, ApiS3Service apiS3Service, ApiProperties apiProperties, EmailAdminService emailAdminService, ParticipanteAdminService participanteAdminService, MensajeAdminService mensajeAdminService, ApiSmsService apiSmsService, CargaAdminService cargaAdminService) {
    ...
    public String inicio(Model model){
        model.addAttribute("productos", reporteAdminService.listarAcciones());
        return ConstantesAdminView.VIEW_METAS_CARGA;
    }

    @PostMapping("viewResumenCargaMetas")
    public String viewResumenCargaMetas(@RequestBody ResultCargaMetas resultCargaMetas, Model model) {
        model.addAttribute("resultCargaMetas", resultCargaMetas);
        return ConstantesAdminView.VIEW_FRAGMENTS_CARGA_RESUMEN_METAS;
    }
    ...
    public PromotickResult importarExcel(@RequestParam("excelfile") MultipartFile excelfile, Auditoria auditoria, PromotickResult promotickResult) {
        ...
        try {
            ...
            CargaMetasValidator cargaMetasValidator = new CargaMetasValidator(cargaExcelAdminService, participanteAdminService, mensajeAdminService, properties, cargaWeb.getIdCarga(), apiSmsService, emailAdminService, auditoria);
            FileValidatorResult<FormCargaMetas, CargaMetas> result = cargaMetasValidator.build(excelfile);
            ...
            if (result.getErrorCountRows() > 0) {
                ...
                Util.getSession().setAttribute(ConstantesSesion.SESSION_CARGA_METAS_ERROR, result.getErrorRows());

                ExcelBuilder.Builder excelBuilder = ExcelBuilder.getInstance(FormCargaMetas.class)
    ...
    public ModelAndView descargarExcelError() {
        return new ModelAndView(
            ExcelBuilder.getInstance(FormCargaMetas.class)
                    .setList((List<FormCargaMetas>) Util.getSession().getAttribute(ConstantesSesion.SESSION_CARGA_METAS_ERROR))
                    .buildView()
        );
    }
}
