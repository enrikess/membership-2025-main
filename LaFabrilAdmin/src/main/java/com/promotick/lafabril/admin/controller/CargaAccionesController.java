...
import com.promotick.lafabril.admin.controller.excel.form.ResultCargaAcciones;
import com.promotick.lafabril.admin.controller.excel.form.ResultCargaMetas;
import com.promotick.lafabril.admin.controller.excel.validator.CargaAccionesValidator;
import com.promotick.lafabril.admin.controller.excel.validator.CargaMetasValidator;
...
public class CargaAccionesController extends BaseController {
    ...
    @Autowired
    public CargaAccionesController(CargaExcelAdminService cargaExcelAdminService, ApiS3Service apiS3Service, ApiProperties apiProperties, EmailAdminService emailAdminService, ParticipanteAdminService participanteAdminService, MensajeAdminService mensajeAdminService, ApiSmsService apiSmsService, CargaAdminService cargaAdminService) {
    ...
    public String inicio(){
        return ConstantesAdminView.VIEW_ACCIONES_CARGA;
    }

    @PostMapping("viewResumenCargaAcciones")
    public String viewResumenCargaAcciones(@RequestBody ResultCargaAcciones resultCargaAcciones, Model model) {
        model.addAttribute("resultCargaAcciones", resultCargaAcciones);
        return ConstantesAdminView.VIEW_FRAGMENTS_CARGA_RESUMEN_ACCIONES;
    }
    ...
    public PromotickResult importarExcel(@RequestParam("excelfile") MultipartFile excelfile, Auditoria auditoria, PromotickResult promotickResult) {
        ...
        try {
            ...
            AbstractResponse<FileMetadata> resultUploadS3 = Util.sentFileToS3(apiS3Service, uploadKey, UtilEnum.TIPO_CARGA.CARGA_META, excelfile, null, true);

            cargaWeb = new CargaWeb()
                    .setIdTipoCarga(1) // meta
            ...
            CargaAccionesValidator cargaAccionesValidator = new CargaAccionesValidator(cargaExcelAdminService, participanteAdminService, mensajeAdminService, properties, cargaWeb.getIdCarga(), apiSmsService, emailAdminService, auditoria);
            FileValidatorResult<FormCargaAcciones, CargaAcciones> result = cargaAccionesValidator.build(excelfile);

            ResultCargaMetas resultCargaMetas = new ResultCargaMetas();
            resultCargaMetas.setRegistrosCorrectos(result.getSuccessfulCountRows());
            resultCargaMetas.setRegistrosError(result.getErrorCountRows());
            resultCargaMetas.setRegistrosTotal(result.getTotalCountRows());
            promotickResult.setData(resultCargaMetas);

            cargaWeb.setCorrectos(resultCargaMetas.getRegistrosCorrectos());
            cargaWeb.setErrores(resultCargaMetas.getRegistrosError());
            cargaWeb.setTotal(resultCargaMetas.getRegistrosTotal());
            cargaWeb.setEstado(resultCargaMetas.getRegistrosError() == 0);
            cargaWeb.setComentario(resultCargaMetas.getRegistrosError() == 0 ? "CARGADO" : "CARGADO CON REGISTROS HA VALIDAR");

            if (result.getErrorCountRows() > 0) {

                Util.getSession().setAttribute(ConstantesSesion.SESSION_CARGA_ACCIONES_ERROR, result.getErrorRows());

                ExcelBuilder.Builder excelBuilder = ExcelBuilder.getInstance(FormCargaAcciones.class)
                ...
                AbstractResponse<FileMetadata> error = Util.sentFileToS3(apiS3Service, excelBuilder.getFilename().replace(apiProperties.getClient().getDirTemporal(), ""), UtilEnum.TIPO_CARGA.CARGA_META_ERROR, null, fileError, true);
    ...
    public ModelAndView descargarExcelError() {
        return new ModelAndView(
            ExcelBuilder.getInstance(FormCargaAcciones.class)
                    .setList((List<FormCargaAcciones>) Util.getSession().getAttribute(ConstantesSesion.SESSION_CARGA_ACCIONES_ERROR))
                    .buildView()
        );
    }
}
